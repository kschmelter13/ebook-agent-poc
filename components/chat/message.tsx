...
function AgentMessageParts({
  ...
  const flushTools = (isSettled: boolean) => {
    ...
    for (const part of partsForGroup) {
      const artifact = getEbookArtifact(part);
      if (artifact) {
        elements.push(<EbookArtifactCard artifact={artifact} key={`artifact:${artifact.artifactId}`} />);
      }
    }
    pendingTools = [];
  };
...
function getEbookArtifact(part: EveDynamicToolPart): EbookArtifact | null {
  ...
  return {
    ...
    coverUrl: typeof output.coverUrl === "string" ? output.coverUrl : undefined,
    pageCount: output.pageCount,
    pdfUrl: output.pdfUrl,
    sourceUrl: output.sourceUrl,
    subtitle: typeof output.subtitle === "string" ? output.subtitle : undefined,
    title: output.title,
  };
}

function EbookArtifactCard({ artifact }: { readonly artifact: EbookArtifact }) {
  return (
    <section className="my-4 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className="flex min-h-40">
        <div className="relative flex w-28 shrink-0 items-center justify-center overflow-hidden bg-[linear-gradient(145deg,#14213d,#2a9d8f)] sm:w-36">
          {artifact.coverUrl ? (
            // The generated Blob URL is returned by the trusted ebook tool.
            // eslint-disable-next-line @next/next/no-img-element
            <img alt="Generated ebook cover" className="absolute inset-0 size-full object-cover" src={artifact.coverUrl} />
          ) : (
            <BookOpenIcon className="size-10 text-white/90" />
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
        </div>
        <div className="flex min-w-0 flex-1 flex-col justify-between gap-4 p-4">
          <div>
            <p className="text-[11px] font-medium tracking-[0.16em] text-emerald-700 uppercase dark:text-emerald-400">
              Ebook ready · {artifact.pageCount} pages
            </p>
            <h3 className="mt-2 text-base font-semibold leading-tight">{artifact.title}</h3>
            {artifact.subtitle ? (
              <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{artifact.subtitle}</p>
            ) : null}
          </div>
          <div className="flex flex-wrap gap-2">
            <Button asChild size="sm">
              <a href={artifact.pdfUrl} rel="noreferrer" target="_blank">
                <DownloadIcon className="size-4" />
                Download PDF
              </a>
            </Button>
            <Button asChild size="sm" variant="outline">
              <a href={artifact.sourceUrl} rel="noreferrer" target="_blank">
                Source JSON
              </a>
            </Button>
          </div>
...
function describeToolAction(part: EveDynamicToolPart, status = getToolStatus(part)) {
  ...
  if (normalized.includes("build ebook")) {
    return status === "running" ? "Designing and rendering the ebook" : "Built the ebook";
  }
  if (normalized.includes("search") || normalized.includes("grep")) {
